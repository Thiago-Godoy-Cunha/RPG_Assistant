...
public static class ClassSkillDataLoader {
    ...
    private static Dictionary<string, JsonElement> Data() {
        ...
        foreach (JsonElement wrapper in doc.RootElement.GetProperty("habilidades de classe").EnumerateArray()) {
    ...
    public static List<Skill> GetAbilities(ClassType type) {
        return GetRaw(type)
            .EnumerateArray()
